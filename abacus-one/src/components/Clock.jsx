...
const Clock = ({ roundStarted, roundEnded, endTime, isActive,onStartGame }) => {
  const [milliseconds, setMilliseconds] = useState(endTime * 1000);
  const [finalTime, setFinalTime] = useState(null);
  const [speed, setSpeed] = useState(null);
  ...
  useEffect(() => {
    ...
    if (isActive) {
      ...
      if (roundEnded.mathResults.correct === 0) {
        ...
        setSpeed(`${(timeTaken / roundEnded.mathResults.correct).toFixed(2)} seconds per math`);
      }
          ...
          {isActive ? (
            ...
            <div id="progress-report" className="font-bold text-xl">
              ...
              <p className="py-2">{`Speed: ${speed}`}</p>
                   <div className=" py-4"><button className="bg-orange-600 animate-bounce text-white font-bold py-2 px-4 rounded text-xl"  onClick={onStartGame}>Start A new Round</button></div>
            </div>
          )}
        </div>
      </div>
